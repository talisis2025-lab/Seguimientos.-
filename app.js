...
const formatTime = value => { if (!value) return "Hora por definir"; const [hour,minute]=String(value).slice(0,5).split(":").map(Number); return new Intl.DateTimeFormat("es-MX",{hour:"numeric",minute:"2-digit"}).format(new Date(2000,0,1,hour,minute)); };
...
function leadCard(l) {
  ...
  return `<article class="lead-card"><span class="status-bar ${s.key}"></span><div class="lead-main"><strong>${escapeHtml(l.name)}</strong><small>${escapeHtml(l.phone)} · ${escapeHtml(l.email)}</small><span class="program-pill">${escapeHtml(l.program)}</span></div><div class="lead-data"><small>ÚLTIMO CONTACTO</small><strong>${fmtShort.format(last)}</strong></div><div class="lead-data"><small>ESTATUS · PRÓXIMO CONTACTO</small><span class="status-pill ${s.key}">${s.label}</span><span class="schedule-detail">${escapeHtml(scheduleOf(l))}</span></div><div class="lead-actions"><button class="contact-btn" data-touch="${l.id}">✓ Contacté hoy</button><button class="delete-btn" data-delete="${l.id}" aria-label="Eliminar prospecto" title="Eliminar">×</button></div></article>`;
}
