...
$("#leadForm").addEventListener("submit", async e => { e.preventDefault(); const leadForm=e.currentTarget; const button=e.submitter; button.disabled=true; const form=Object.fromEntries(new FormData(leadForm)); form.id=crypto.randomUUID(); try { await api("create", { lead: form }); leadForm.reset(); leadForm.program.value="Presencial"; $("#contactDate").value=localISO(); showToast("Prospecto registrado correctamente."); await loadLeads(); $$('.tab')[1].click(); } catch(error){ showToast(error.message,true); if(!state.apiUrl) openSettings(); } finally { button.disabled=false; } });
