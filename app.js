...
async function api(action, payload = {}) {
  ...
  const response = await fetch(state.apiUrl, { method: "POST", redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ action, ...payload }) });
  if (!response.ok) throw new Error("No fue posible conectar con Google Sheets.");
  const data = await response.json();
  if (!data.ok) throw new Error(data.error || "Ocurrió un error en Google Sheets.");
  return data;
}
...
$("#leadForm").addEventListener("submit", async e => { e.preventDefault(); const button=e.submitter; button.disabled=true; const form=Object.fromEntries(new FormData(e.currentTarget)); try { await api("create", { lead: form }); e.currentTarget.reset(); e.currentTarget.program.value="Presencial"; $("#contactDate").value=localISO(); showToast("Prospecto registrado correctamente."); await loadLeads(); $$('.tab')[1].click(); } catch(error){ showToast(error.message,true); if(!state.apiUrl) openSettings(); } finally { button.disabled=false; } });
$("#leadList").addEventListener("click", async e => { const touch=e.target.closest("[data-touch]"); const del=e.target.closest("[data-delete]"); if(touch){ touch.disabled=true; try{ await api("touch",{id:touch.dataset.touch}); showToast("Contacto actualizado a hoy."); await loadLeads(); }catch(error){showToast(error.message,true)}finally{touch.disabled=false} } if(del && confirm("¿Eliminar este prospecto definitivamente?")){ try{await api("delete",{id:del.dataset.delete});showToast("Prospecto eliminado.");await loadLeads()}catch(error){showToast(error.message,true)} } });
