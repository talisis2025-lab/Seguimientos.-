...
function apiRead() {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.src = `${state.apiUrl}${separator}action=list&callback=${encodeURIComponent(callback)}&_=${Date.now()}`;
    document.head.appendChild(script);
  });
}

async function apiWrite(action, payload = {}) {
  if (!state.apiUrl) throw new Error("Primero conecta tu Google Sheet desde el engrane.");
  await fetch(state.apiUrl, { method: "POST", mode: "no-cors", redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ action, ...payload }) });
...
async function loadLeads() {
  ...
  try { state.leads = (await apiRead()).leads || []; render(); }
...
$("#leadForm").addEventListener("submit", async e => { e.preventDefault(); const button=e.submitter; button.disabled=true; const form=Object.fromEntries(new FormData(e.currentTarget)); form.id=crypto.randomUUID(); try { await apiWrite("create", { lead: form }); const data=await apiRead(); state.leads=data.leads||[]; if(!state.leads.some(l=>l.id===form.id)) throw new Error("Apps Script recibió la solicitud, pero no confirmó el registro. Revisa sus permisos."); e.currentTarget.reset(); e.currentTarget.program.value="Presencial"; $("#contactDate").value=localISO(); showToast("Prospecto registrado correctamente."); render(); $$('.tab')[1].click(); } catch(error){ showToast(error.message,true); if(!state.apiUrl) openSettings(); } finally { button.disabled=false; } });
$("#leadList").addEventListener("click", async e => { const touch=e.target.closest("[data-touch]"); const del=e.target.closest("[data-delete]"); if(touch){ touch.disabled=true; try{ await apiWrite("touch",{id:touch.dataset.touch}); await loadLeads(); showToast("Contacto actualizado a hoy."); }catch(error){showToast(error.message,true)}finally{touch.disabled=false} } if(del && confirm("¿Eliminar este prospecto definitivamente?")){ try{await apiWrite("delete",{id:del.dataset.delete});await loadLeads();showToast("Prospecto eliminado.")}catch(error){showToast(error.message,true)} } });
